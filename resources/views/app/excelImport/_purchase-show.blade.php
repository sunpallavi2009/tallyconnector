...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('excelImport.purchase.create') }}">{{ __('Purchase Invoices Import') }}</a></li>
    <li class="breadcrumb-item">{{ __('View Purchase Invoices') }}</li>
@endsection
@section('content')
    <div class="row">
        <div class="col-sm-12">
                ...
                <div class="col-xl-12">
                    <div class="card sticky-top">
                        <div class="list-group list-group-flush" id="useradd-sidenav">
                            @include('app.excelImport.sidebar._purchase-sidebar')
                        </div>
                    </div>
                </div>
                            ...
                            <div class="card-header">
                                <div class="row">
                                    <div class="col-lg-8 d-flex align-items-center">
                                        <h5 class="mb-0">{{ __('Purchase Invoices Data') }}</h5>
                                    </div>
...
@push('javascript')
    ...
    <script>
        $(document).on('change', '.edit--placeofsupply-select', function() {
            var newValue = $(this).val();
            var id = $(this).data('id');
            var url = '{{ route("salespurchase.input.store", ":id") }}';
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#salePurchase-table').DataTable().ajax.reload(); // Reload DataTable after successful update
                },
    ...
    <script>
        $(document).on('change', '.edit-vouchertype-select', function() {
            var newValue = $(this).val();
            var id = $(this).data('id');
            var url = '{{ route("salespurchase.input.store", ":id") }}';
