...
use App\DataTables\App\CreditNoteDataTable;
...
class CreditNoteController extends Controller
{
    ...
    {
        return $dataTable->render('app.credit-note.index');
    }

    public function creditNoteImport(Request $request)
    {
        $request->validate([
            'file' => 'required|file'
        ]);
        ...
        if ($valid) {
            ...
            return redirect()->route('credit-note.show')->with('success', __('Credit Note Data Save Successfully.'));
        } else {
            return redirect()->back()->with('error', 'Data validation failed.');
        }
    }

    public function creditNoteShow(CreditNoteDataTable $dataTable)
    {
        return $dataTable->render('app.credit-note._show');
    }

    public function creditNoteDestroy($id)
    {
        $credit = SalePurchaseInvoice::find($id);
        $credit->delete();
        return redirect()->route('credit-note.show')->with('success', __('Credit Note Data deleted successfully'));
    }
}
